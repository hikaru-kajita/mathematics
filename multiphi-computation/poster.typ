...
特に後者については, primechainにアクセスするときの添字が素数であることから, wheel sieveの考え方を利用して空間計算量を $log log N$ だけ落とすことができる.

= テスト

正当性のテストのため, RAMに入る範囲の $N=10^8, k=4$ で通常のエラトステネスの篩と比較し, $1<=n<=N$ の範囲で $phi(n)$ から $phi^4 (n)$ までの値がすべて一致することを確認した.
