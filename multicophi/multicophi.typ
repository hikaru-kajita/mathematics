...
また, $k$ は $n$ に依らない定数とし, 以降は $k>=2$ の場合について考える.

#theorem[
  $k>=1$ とする. $"co"phi^k (n)<=0$ ならば, $phi^(k-1) (n)=1$.
] <comultiphi-nonpositive>

#proof[
  対偶法で証明する. つまり, まず $phi^(k-1) (n)>=2$ と仮定する.

  すると, $phi(n), phi^2 (n), phi^3 (n), ..., phi^(k-1) (n)$ はすべて2以上の整数である.
  ...
  ここで $phi(n), phi^2 (n), ..., phi^(k-1) (n)$ がすべて偶数なので, $display(phi^k (n)<=phi(n)product_(1<=j<k)(1-1/2)=phi(n)/(2^(k-1)))$.
...
#proof[
  $"co"phi^k (n)=C<=0$ とすると, 先の命題より $phi^(k-1) (n)=1$ から $phi^k (n)=1$.

  したがって, $"co"phi^k (n)=n-2^(k-1)phi^k (n)=n-2^(k-1)$ より, $n=2^(k-1)+C$ と書ける.

  次に, これが実際に $"co"phi^k (n)=C$ の解であることを示そう.

  先の命題の証明の中で, $phi^(k-1) (n)>=2$ ならば $2^(k-1)phi^k (n)<n$ を示していた.

  したがって, $phi^(k-1) (n)>=2$ ならば $2^(k-1)<n$.

  しかし, いま $n=2^(k-1)+C<=2^(k-1)$ より, $phi^(k-1) (n)=1$ から $phi^k (n)=1$.
